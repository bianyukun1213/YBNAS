...
namespace YBNAS
{
    ...
    internal class SigninTask
    {
        ...
        private async Task<User> Auth(string vr, string csrfToken, string userAgent)
        {
            ...
            if (authResCode != 0)
            {
                _logger.Error($"{GetLogPrefix()}：校本化认证失败，服务端返回消息：{authResMsg}。");
                return new User();
            }
        ...
        private async Task<Device> GetDevice(string csrfToken, string userAgent)
        {
            ...
            if (deviceResCode != 0)
            {
                _logger.Error($"{GetLogPrefix()}：获取授权设备失败，服务端返回消息：{deviceResMsg}。");
                return new Device();
            }
        ...
        private async Task<SigninInfo> GetSigninInfo(string csrfToken, string userAgent)
        {
            ...
            if (infoResCode != 0)
            {
                _logger.Error($"{GetLogPrefix()}：获取签到信息失败，服务端返回消息：{infoResMsg}。");
                return new();
            }
        ...
        private async Task<bool> Signin(string csrfToken, Device device, string userAgent)
        {
            ...
            if (signinResCode != 0)
            {
                _logger.Error($"{GetLogPrefix()}：签到失败，服务端返回消息：{signinResMsg}。");
                return false;
            }
            return true;
        }
